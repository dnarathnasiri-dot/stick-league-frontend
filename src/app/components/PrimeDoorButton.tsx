...
export function PrimeDoorButton({ variant, label, onComplete, className = "" }: { variant: "login" | "logout" | "admin"; label: string; onComplete: () => void; className?: string }) {
  ...
  const run = () => {
    if (variant === "login" || variant === "admin") {
    ...
    window.setTimeout(() => {
      setClasses("clicked door-slammed"); setButtonState("walking2");
      window.setTimeout(() => {
        setClasses("clicked door-slammed falling"); setButtonState("falling1");
        window.setTimeout(() => {
          setButtonState("falling2");
          window.setTimeout(() => {
            setButtonState("falling3");
            window.setTimeout(() => { setClasses(""); setButtonState("default"); onComplete(); }, 700);
          }, 300);
        }, 400);
      }, 400);
    }, 300);
