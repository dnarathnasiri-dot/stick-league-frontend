...
export type DoorFeature = "esport" | "questboard" | "football";
...
export function DoorsPage({
  ...
  const handleTime =
    (isA: boolean) => (e: React.SyntheticEvent<HTMLVideoElement>) => {
      if (isA !== activeARef.current || lockRef.current) return;
      const cur = e.currentTarget;
      if (!isFinite(cur.duration)) return;
      const lead = isA ? 0.04 : FADE_MS / 1000;
      const fade = isA ? 120 : FADE_MS;
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-[#efe9da] text-[#2b2b2b]">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-[1320px] flex-col px-6 py-6 lg:px-12">
        ...
        {/* Door Hotspots — 3 doors */}
        <div className="pointer-events-none absolute inset-0 z-20">
          <DoorHotspot
            feature="esport"
            label="E-Sports"
            emoji="🎮"
            className="left-[24%] top-[54%] h-[48%] w-[20%]"
          ...
          <DoorHotspot
            ...
            label="Quest Board"
            emoji="📋"
            className="left-[50%] top-[52%] h-[52%] w-[22%]"
            onDoorSelect={onDoorSelect}
          />
          <DoorHotspot
            feature="football"
            label="Football"
            emoji="⚽"
            className="left-[76%] top-[54%] h-[48%] w-[20%]"
            onDoorSelect={onDoorSelect}
          />
        </div>
